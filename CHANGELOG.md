...
- **Canvas / Runtime**: Migrated all Canvas scene mounting from `onMount` and `bind:this` to Svelte attachments while keeping live uniform updates on the existing WebGL scene.
- **Canvas / Cleanup**: Added deterministic cleanup for animation frames, event listeners, OGL meshes, programs, geometries, textures, render targets, MediaPipe resources, and camera streams.

### Fixed

- **Canvas / Live Controls**: Prevented attachment initialization from tracking component props, so changing uniform controls no longer recreates the WebGL scene, resets animation state, or flashes a black frame.
...
- **Canvas / Glass Pane**: Replaced the previous `distortion`, `waviness`, `frequency`, and `rods` controls with panel-focused controls: `rotation`, `refraction`, `chromaticAberration`, `panelWidth`, `waveFrequency`, `waveAmplitude`, and `speed`.
...
- **Canvas / Globe, Halo, Lava Lamp, Rubiks Cube**: Added shared `scale`, `rotation`, `offsetX`, and `offsetY` transform controls aligned with Glass Logo.
- **Canvas / Globe**: Removed the redundant `radius` prop in favor of `scale`, synchronized marker tooltips with transform offsets, and added pulsing shader markers.
- **Canvas / Halo**: Removed the redundant `fov` prop in favor of `cameraDistance`.
- **Canvas / Rubiks Cube**: Removed the redundant `size` prop in favor of `scale`, adjusted offset behavior to viewport space, and reduced perspective distortion with a narrower camera FOV.

### Fixed

- **Canvas / Lava Lamp**: Corrected raymarch SDF scaling so blobs remain stable when `scale` is below `1`.
